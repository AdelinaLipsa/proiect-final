...
$(document).ready(function () {
  var sizePicker = $('#size-picker');
  var canvas = $('#my-node');
  var body = $('body');
  var removeGrid = false;
  var gridColor = $('#grid-color');
  var currentColor = $('#colorPicker');
  ...
  var twelve = $(".twelve");
  var sixteen = $(".sixteen");
  var thirtytwo = $(".thirtytwo"); //when mouse button is held down the mousedragged variable is set to true
  ...
  body.on('click', '#paint-brush', function () {
    ...
    removeCurrentTool();
    currentTool = "paintBrush"; //adds the tool-selected class to div which gives an orange shadow
  ...
  body.on('click', '#paint-bucket', function () {
    //removes orange shadow on div from the last used tool
    removeCurrentTool();
    currentTool = "paintBucket"; //adds the tool-selected class to div which gives an orange shadow
  ...
  body.on('click', '#eye-dropper', function () {
    //removes orange shadow on div from the last used tool
    removeCurrentTool();
    currentTool = "eyeDropper"; //adds the tool-selected class to div which gives an orange shadow
  ...
  body.on('click', '#eraser', function () {
    //removes orange shadow on div from the last used tool
    removeCurrentTool();
    currentTool = "eraser"; //adds the tool-selected class to div which gives an orange shadow
  ...
  canvas.on('mousemove', function (e) {
    ...
    if (mouseDragged) {
      ...
      var clickedCell = $(e.target).closest('td'); //continuously paint the nearest cell

      if (currentTool === "paintBrush") {
        highlightSquares(clickedCell); //countinously erase the nearest cell
  ...
  canvas.on('click', 'td', function (e) {
    ...
    switch (currentTool) {
      ...
      case "eyeDropper":
        removeCurrentTool();
        eyeDropper(clickedCell); //changes currentTool to paintbrush once eyedropper gets color from cell
  ...
  function makeGrid() {
    ...
    canvas.empty(); //converts the height and width from string values to a number
    //loops through the height value until it creates the same number of rows

    for (var i = 0; i < height; i++) {
      //adds a new row
      canvas.append($('<tr />').attr('data-row', i)); //stores the location of the current row for easy access for the next for loop

      currentRow = canvas.children().last(); //loops through the width value until it creates the same number of columns

      for (var _i = 0; _i < width; _i++) {
        //adds a new column
        currentRow.append($('<td bgcolor="white"></td>').attr('data-col', _i));
      }
    }
  ...
  function paintBucket(chosenCell) {
    ...
    var colNum = chosenCell.data('col');
    var rowNum = chosenCell.parent().data('row'); //checks to see if there is a cell to the left of chosenCell

    if (colNum > 0) {
      var leftCell = chosenCell.prev(); //checks to see if the cell to the left is the same color as the oldFillColor
    ...
    if (colNum < width) {
      var rightCell = chosenCell.next(); //checks to see if the cell to the right is the same color as oldFillColor
    ...
    if (rowNum > 0) {
      var topCell = chosenCell.parent().prev().children().eq(colNum); //checks to see if the cell above is the same color as the oldFillColor
    ...
    if (rowNum < height) {
      var bottomCell = chosenCell.parent().next().children().eq(colNum); //checks to see if the cell below the chosenCell is the same color as oldFillColor
  ...
  function eyeDropper(clickedCell) {
    var tempColor = clickedCell.css('background-color'); //sets the color of currentColor to the color of the given cell
  ...
  $('.secondFoo').click(function () {
    html2canvas($('#my-node'), {
      onrendered: function onrendered(canvas) {
        var a = document.createElement('a'); // toDataURL defaults to png, so we need to request a jpeg, then convert for file download.
